...
def doFile(filename)
  ...
  fileArray.each do |file|
    ...
    (text.last["INF"][2].to_i).times do |n|
      ...
      summaryText << Summarise.text(filename, text_subsection)
      summaryText << "\n\n"
    end

    # replace existing summary text
    #summaryText << "\n"
    n = 0
    s = 0 
    info.each_with_index do |e,i|
      if e =~ /\Asummary_text/
        n = (info.length - i) - 1
        s = i + 1
      end
    ...
    tmpHsh = {}
    tmpHsh["TXT"] = info
    FileIO.hashToFile(tmpHsh, ",", file, "w")
    text.each_with_index do |e,i|
      FileIO.hashToFile(e, ",", file, "a")
    end
    puts ""
